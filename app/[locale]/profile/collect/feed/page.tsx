          ...
          <div key={i} className={"h-28   mb-4 flex justify-between"}>
            ...
            <div className="flex flex-col justify-between">
              ...
              <div className="flex items-center">
                <div className={`w-6 h-6 rounded-full mr-2 ${v.avtar} bg-cover `}></div>
                <span className="text-text-theme text-xs">{v.name}</span>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <>
      <div className="total-num p-4">
        <span className="text-gray-400">{t("total")}：</span>9999999
      </div>
      {Posts()}
    </>
  )
