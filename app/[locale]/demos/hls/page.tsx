...
export default function HLSDemo() {
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      // Fix source URL to use our API route that handles dynamic files
      const apiSrc = hlsUrl.replace("/uploads/hls/", "/api/hls/")
      console.log("Loading HLS from API source:", apiSrc)

      // Load the HLS stream
      hls.loadSource(apiSrc)
