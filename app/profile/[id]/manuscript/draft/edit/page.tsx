    ...
    <>
      ...
      >
          ...
          <section className="flex flex-col gap-5 mt-2">
            {
              itemsList.map((field, index) => {
                return (
                  <Controller key={field.id} name={`items.${index}.content`} control={control}
                    render={({ field }) => {
                      return (
                        <InputWithLabel value={field.value}
        ...
        <section className="px-4">
          <Controller control={control} render={({ field }) => {
            return (
              <FormItemWithSelect onValueChange={field.onChange} label={"单选/多选"} value={field.value}
          ...
          <section className="flex justify-between items-center border-b border-[#ddd] py-4">
            ...
            <Controller control={control} render={({ field }) => {
              return (
                <DatePickerModal onValueChange={(value) => {
      ...
      >
        {
          priceList.map((item, index) => {
            return (
              <section className={clsx("px-4 mt-5", disableOption2 && index === 1 ? "opacity-40" : "")}
                key={item.id}
              >
                <h3>付费设置{index + 1} {disableOption2 && index === 1 ? "(无效)" : ""}</h3>
                <Controller render={({ field }) => {
                  return (
                    <FormItemWithSelect label={"付费对象"} value={field.value}
                ...
                <section className={"relative"}>
                  <section className="flex justify-between items-center border-b border-[#ddd] py-4">
                    ...
                    <Controller control={control} render={({ field }) => {
                      return (
                        <input className="flex-1 text-right" min={0} value={field.value}
                          onChange={event => {
                  ...
                  <section
                    className="text-xs text-red-600 absolute right-4 bottom-0"
                  >{errors.priceList?.[index]?.price?.message}</section>
                </section>
  ...
  return (
    <>
      {itemsList?.map((item, index) => {
        return (
          <Controller control={control} key={item.file_id} render={({ field }) => {
            return (
              <div
                key={item.file_id}
              ...
              >
                ...
                >
                  <Image alt={"delete"} src={"/icons/profile/icon-delete.png"} width={16} height={16}/>
                </button>
              </div>
            )
    ...
    <FormProvider {...postForm}>
      <form onSubmit={handleSubmit(onFormSubmit)}>
        <section className="flex justify-between h-11 items-center pl-4 pr-4 ">
          ...
          />
          <button type="submit"
            className={clsx(!formState.isValid ? "text-[#bbb]" : "#000")}
          >发布
          </button>
        </section>
        ...
        <section className="pt-5 pb-5 pl-4 pr-4 border-b border-gray-200 relative">
          ...
          />
          <div
            className="absolute left-4 bottom-1.5 text-red-600 text-xs"
          >{formState?.errors?.post?.title?.message}</div>
        </section>
        {
          watch("post_vote") !== undefined && (
            <section className="pt-5 pb-5 pl-4 pr-4 border-b border-gray-200">
              ...
              <section className="mt-2.5 rounded-xl bg-[#F4F5F5] px-3 py-2">
                ...
                <div
                  className="text-xs text-[#999] mt-1.5"
                >截止：{formValues.post_vote?.stop_time ? dayjs(formValues.post_vote?.stop_time).format("YYYY-MM-DD HH:mm") : ""} 结束
                </div>
              </section>
            </section>
          )
        ...
        <section className="pt-5 pb-5 pl-4 pr-4 border-b border-gray-200">
          ...
          <section className="mt-2.5">
            {formValues.post_price?.map((price, index) => (
              <ReadingSettingsDisplay postPrice={price}
                key={index}
              />
            ))}
