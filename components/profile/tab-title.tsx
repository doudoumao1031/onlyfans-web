...
export default function TabTitle({tabOptions, active, activeChange}: {
    ...
    return <div className="grid grid-cols-2 border-b border-gray-100">
        {tabOptions.map((item, index) => (
            <button className={clsx("pt-3.5 pb-3.5 text-[20px] relative", activeClass(item.name))} onTouchEnd={() => {
                activeChange(item.name)
            }} key={index}>{item.label} <span className={clsx("absolute left-[50%] bottom-0 h-[3px] rounded-tl-lg rounded-tr-lg bg-black w-[40px] ml-[-20px]", active === item.name ? "block" :"hidden")}></span></button>))}
    </div>
}