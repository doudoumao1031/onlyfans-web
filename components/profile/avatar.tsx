...
export default function Avatar({showEdit, showLive,fileId}: { showEdit?: boolean, showLive?: boolean ,fileId:string}) {
    return <div className="absolute rounded-full p-0.5 bg-white w-[90px] h-[90px] top-[-47px] left-[50%] ml-[-45px] ">
        <section className="w-full h-full relative rounded-full">
            ...
            {
                showEdit && <button type={"button"} className="absolute rounded-bl-full rounded-br-full left-0 bottom-0 h-[40px] w-full bg-[rgba(0,0,0,0.5)] text-white flex items-center justify-center">
                <IconWithImage
