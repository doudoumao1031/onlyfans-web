...
export default function RechargePanel({ amount }: {amount: number}) {
  ...
  return (
    <div className={"p-4"}>
      <div
        className={"bg-[url('/theme/bg_wallet@3x.png')]  bg-cover rounded-xl text-white flex justify-between items-center px-[20px] pt-[10px] pb-[20px]"}
      >
        <div className={"flex flex-col flex-1 overflow-hidden"}>
          <span className={"text-xs"}>{commonT("appName")}{t("balance")}</span>
          <div className={"flex items-baseline font-medium"}>
            <span className={"text-[32px] text-ellipsis whitespace-nowrap overflow-hidden "}>{initAmount || 0.00}</span>
