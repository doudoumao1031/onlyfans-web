...
export default function Page({ children = null, visible = false, closeModal = true, type, cancelText, okText, title, content, footer, cancel = () => { }, confirm = () => { } }: TModaProps) {
    ...
    const DefaultFooter = () => {
        return <div className="w-full flex items-center justify-center text-[16px] border-t border-[#ebeced] h-[44px]">
            <div onClick={() => { cancel() }} className="h-full flex justify-center items-center flex-1 text-center text-[#6D7781] border-r border-[#ebeced]">{cancelText || '取消'}</div>
            <div onClick={() => { confirm() }} className="h-full flex justify-center items-center flex-1 text-center text-main-pink">{okText || '确定'}</div>
        </div>
    }
    return <div onClick={() => { closeModal && cancel() }} className="fixed top-0 left-0 w-screen h-screen bg-black/50 z-50 flex justify-center items-center">
        {children}
        {type === 'modal' && <div className="w-10/12  rounded-lg bg-white" onClick={(e) => { e.stopPropagation() }}>
